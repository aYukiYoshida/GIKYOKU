...
const getSubLocator = async (
  locator: Locator,
  subLocator?: Locator,
  text?: string | RegExp,
): Promise<Locator> => locator.filter({ has: subLocator, hasText: text });
  ...
  if (subSelector) {
    ...
    if (subSelector[1]?.subSelector) {
      resolvedLocator = await subLocatorLookup({
        ...
        timeout: subSelector[1]?.timeout,
        subSelector: subSelector[1]?.subSelector,
        state: subSelector[1]?.state,
      });
    }
  }
...
export const recursiveLocatorLookup = async ({
  ...
  const locator =
    typeof selector === "string"
      ? page.locator(selector, { hasText: options?.hasText })
      : await getSubLocator(selector, undefined, options?.hasText);
